...
export const MagicLinkBanner: React.FC<MagicLinkBannerProps> = ({
  ...
  if (isMagicLinkMode) {
    return (
      <div className="bg-slate-900 text-white border-b border-slate-800 py-2 px-4 dir-rtl">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2">
            <KeyRound className="w-4 h-4 text-amber-400 shrink-0" />
            <span className="text-amber-300 font-bold">גישת לקוח פעיל מתוך גיליון (Magic Link):</span>
            <span className="font-bold text-slate-200">{customer.company} ({customer.name})</span>
            <span className="font-mono bg-slate-800 text-blue-300 px-2 py-0.5 rounded text-[11px] border border-slate-700">
              {customer.id}
            </span>
          </div>
          <div className="text-[11px] text-emerald-400 font-bold flex items-center gap-1">
            <Sparkles className="w-3 h-3 text-emerald-400" />
            <span>סנכרון מלא ל-Google Sheets & Drive</span>
          </div>
        </div>
      </div>
    );
